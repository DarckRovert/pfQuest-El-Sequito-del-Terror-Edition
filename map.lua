...
-- this loop puts it into one place and only updates it every .05 seconds
...
controlkey:SetScript("OnUpdate", function()
  if (this.throttle or 0.05) > GetTime() then return else this.throttle = GetTime() + 0.05 end
  if WorldMapFrame:IsShown() and MouseIsOver(WorldMapFrame) or MouseIsOver(pfMap.drawlayer) then
    ...
    -- Clear key state when outside relevant frames to prevent stale state
