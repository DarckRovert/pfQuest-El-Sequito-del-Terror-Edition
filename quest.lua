...
-- a different zone header when their real zone header is collapsed (vanilla API quirk).
...
pfQuest:SetScript("OnEvent", function()
  ...
  if event == "QUEST_LOG_UPDATE" then
    -- Keep data.collapsed in sync with collapsedQuestIDs.
...
pfQuest:SetScript("OnUpdate", function()
  ...
  for id, entry in pairs(this.queue) do
    ...
    if entry[4] == "REMOVE" then
      ...
      -- remove from collapsed tracking so the SavedVar doesn't accumulate stale questids
