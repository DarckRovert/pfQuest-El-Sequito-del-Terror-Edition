-- ============================================================
--  pfQuest :: Extend Module :: QuestHelper Main Logic
--  Fuente: Cliencer/pfExtend v1.0.5
--  Adaptado para integración en pfQuest - El Séquito del Terror
-- ============================================================
...
PFEXQuestHelper.FindPreUndo = function(id)
    if quests[id]["pre"] then
        local one_complete, level, thislevel = nil, 0, 0
        for _, prequest in pairs(quests[id]["pre"]) do
            ...
            if not pfQuest_history[prequest] then
                ...
                thislevel = 2
                if not flag.WRONGRACE and not flag.WRONGCLASS then thislevel = 3 end
            end
...
PFEXQuestHelper.QuestFilter = function(id)
    local ret = {
        min  = 999, lvl = 999,
        DOING = false, FINISHED = false, AFTERFINISHED = false,
        UNKNOWN = false, HASPRE = false, UNDOPRE = false,
        WRONGRACE = false, WRONGCLASS = false, WRONGSKILL = false,
        WRONGFACTION = false, LOWLEVEL = false, EVENT = false,
        STARTUNIT = false, STARTOBJECT = false, STARTITEM = false,
    }
    ...
    if quests[id]["start"] then
        ...
        if quests[id]["start"]["I"] then ret.STARTITEM = true end
    end
    return ret
end
...
PFEXQuestHelper.ReadCoords = function(id, type)
    local ret  = {}
    local data = { ["U"] = units, ["V"] = units, ["O"] = objects }
...
PFEXQuestHelper.GetStartZones = function(id)
    ...
    if quests[id]["start"] then
        if quests[id]["start"]["U"] then
            for _, unit in pairs(quests[id]["start"]["U"]) do
                for _, v in ipairs(PFEXQuestHelper.ReadCoords(unit, "U")) do table.insert(ret, v) end
            end
        end
        if quests[id]["start"]["O"] then
            for _, object in pairs(quests[id]["start"]["O"]) do
                for _, v in ipairs(PFEXQuestHelper.ReadCoords(object, "O")) do table.insert(ret, v) end
            end
        end
            ...
            for _, item in pairs(quests[id]["start"]["I"]) do
                if items[item] then
                    for lootType, lootSources in pairs(items[item]) do
                        if lootType == "R" then
                            for id2, chance in pairs(lootSources) do
                                if refloot[id2] then
                                    for refLootType, refLootSources in pairs(refloot[id2]) do
                                        for refId, _ in pairs(refLootSources) do
                                            for _, v in ipairs(PFEXQuestHelper.ReadCoords(refId, refLootType)) do table.insert(ret, v) end
                                        end
                                    end
                                end
                            end
                        ...
                        if lootType == "V" or lootType == "U" or lootType == "O" then
                            for id2, chance in pairs(lootSources) do
                                for _, v in ipairs(PFEXQuestHelper.ReadCoords(id2, lootType)) do table.insert(ret, v) end
                            end
                        end
                    end
                end
...
PFEXQuestHelper.UpdateDatabase = function()
    ...
    for questId, data in pairs(quests) do
        ...
        if PfExtend_Database["QuestHelper"]["QuestAfter"][questId] == nil then
            PfExtend_Database["QuestHelper"]["QuestAfter"][questId] = {}
        end
        if data["pre"] then
            for _, pre in pairs(data["pre"]) do
                if PfExtend_Database["QuestHelper"]["QuestAfter"][pre] == nil then
                    PfExtend_Database["QuestHelper"]["QuestAfter"][pre] = {}
                end
...
PFEXQuestHelper.findZoneByLocation = function(location)
    if zones[location] then return zones[location][1] end
end

PFEXQuestHelper.findLocationByZone = function(Zone)
    local ret = {}
    for k, v in pairs(zones) do
        if v[1] == Zone then table.insert(ret, k) end
    end
    return table.unique(ret)
end

-- Botón QH en el mapa del mundo
PFEXQuestHelper.MapToggleButton = CreateFrame("Button", "PFEXQuestHelperMapToggleButton", WorldMapFrame, "UIPanelButtonTemplate")
...
function PFEXQuestHelper.QuestChainBuilder(questList)
    ...
    for _, id in ipairs(questList) do MarkParents(id) end
    for _, id in ipairs(questList) do
        if not hasParent[id] then table.insert(roots, id) end
    end
    ...
    local function BuildNode(id)
        if processed[id] then return nil end
        processed[id] = true
        local node = { id = id, flag = PFEXQuestHelper.QuestFilter(id), children = {} }
    ...
    for _, rootId in ipairs(roots) do
        ...
        if tree then table.insert(result, tree) end
    end
    ...
    local function CalculateAfterFinished(node)
        ...
        for _, child in ipairs(node.children) do
            ...
            if not child.flag.AFTERFINISHED then allChildrenFinished = false end
        end
        if not hasChildren then
            node.flag.AFTERFINISHED = node.flag.FINISHED or node.flag.WRONGSKILL or node.flag.WRONGCLASS or node.flag.WRONGRACE
        else
            node.flag.AFTERFINISHED = (node.flag.FINISHED or node.flag.WRONGSKILL or node.flag.WRONGCLASS or node.flag.WRONGRACE) and allChildrenFinished
        end
    end
    for _, tree in ipairs(result) do CalculateAfterFinished(tree) end

    local PRIORITY = { HAS_PRE = 2, FINISHED = 0, AFTER_FIN = 4, EVENT = 8, WRONG_SKILL = 16, WRONG_CLASS = 32, WRONG_RACE = 64, UNKNOWN = 128 }
    local function GetPriorityScore(flag, hasPre)
        local score = 0
        if hasPre              then score = bit.bor(score, PRIORITY.HAS_PRE)    end
        if flag.FINISHED       then score = bit.bor(score, PRIORITY.FINISHED)   end
        if flag.AFTERFINISHED  then score = bit.bor(score, PRIORITY.AFTER_FIN)  end
        if flag.EVENT          then score = bit.bor(score, PRIORITY.EVENT)      end
        if flag.WRONGSKILL     then score = bit.bor(score, PRIORITY.WRONG_SKILL) end
        if flag.WRONGCLASS     then score = bit.bor(score, PRIORITY.WRONG_CLASS) end
        if flag.WRONGRACE      then score = bit.bor(score, PRIORITY.WRONG_RACE) end
        if flag.UNKNOWN        then score = bit.bor(score, PRIORITY.UNKNOWN)    end
        return score
    end

    local function SortTree(node, hasPre)
        for _, child in ipairs(node.children) do SortTree(child, true) end
        if table.countNum(node.children) > 1 then
            table.sort(node.children, function(a, b)
                local sA = GetPriorityScore(a.flag, true)
                local sB = GetPriorityScore(b.flag, true)
                if sA ~= sB then return sA < sB end
                return (a.flag.lvl or 999) < (b.flag.lvl or 999)
            end)
        end
    end
    for _, tree in ipairs(result) do SortTree(tree, false) end

    if table.countNum(result) > 1 then
        table.sort(result, function(a, b)
            local sA = GetPriorityScore(a.flag, false)
            local sB = GetPriorityScore(b.flag, false)
            if sA ~= sB then return sA < sB end
            return (a.flag.lvl or 999) < (b.flag.lvl or 999)
        end)
    end
...
PFEXQuestHelper.OnMapChange = function()
    ...
    if z2q[PFEXQuestHelper.zone] then
        for k, _ in pairs(z2q[PFEXQuestHelper.zone]) do table.insert(questList, k) end
    end
...
function PFEXQuestHelper.OnLoad() end

local zone, last_zone
function PFEXQuestHelper.OnEvent(event, arg1)
    if not PfExtend_Global.ReadSetting("QuestHelper", "enable") then
        if PFEXQuestHelper.MapToggleButton then PFEXQuestHelper.MapToggleButton:Hide() end
        if PFEXQuestHelper.Browser         then PFEXQuestHelper.Browser:Hide() end
        return
    else
        if PFEXQuestHelper.MapToggleButton:IsShown() and PFEXQuestHelper.Browser and PFEXQuestHelper.Browser:IsShown() then
            PFEXQuestHelper.MapToggleButton:Hide()
        elseif not (PFEXQuestHelper.MapToggleButton:IsShown() or (PFEXQuestHelper.Browser and PFEXQuestHelper.Browser:IsShown())) then
            PFEXQuestHelper.MapToggleButton:Show()
        end
    end
    ...
    if event == "PLAYER_ENTERING_WORLD" then
        PFEXQuestHelper.Reload()
        PFEXQuestHelper.GetPlayerData()
        if not PfExtend_Database["QuestHelper"]["updated"] or
