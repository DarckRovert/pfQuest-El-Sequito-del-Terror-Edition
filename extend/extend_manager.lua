-- ============================================================
--  pfQuest :: Extend Module :: Config Manager (ventana unificada)
--  Fuente: Cliencer/pfExtend v1.0.5 - modules/manager.lua
...
-- ShowTooltip y EnableTooltips ahora se cargan desde compat_client.lua
...
pfExtendConfig.title:SetText("|cff33ffccpf|rQuest-" .. pfExtend_Loc["windowTitle_Config"])
...
pfExtendConfig.close.texture:SetTexture(pfExtend_Path .. "\\extend\\compat\\close")
...
pfExtendConfig.close:SetScript("OnClick", function() this:GetParent():Hide() end)
EnableTooltips(pfExtendConfig.close, { pfExtend_Loc["Close"], pfExtend_Loc["Hide browser window"] })
...
pfExtendConfig.CaptionsFrame.backdrop = CreateFrame("Frame", "pfExtendConfigCaptionsBackdrop", pfExtendConfig.CaptionsFrame)
...
pfExtendConfig.CaptionsFrame.list = pfUI.api.CreateScrollChild("pfExtendConfigCaptionsScroll", pfExtendConfig.CaptionsFrame)
pfExtendConfig.CaptionsFrame.list:SetWidth(180)

local function ButtonLeave()
    if pfExtendConfig.selectState then pfExtendConfig.selectState = "clean" end
...
CaptionClick = function(caption)
    for id, frame in pairs(pfExtendConfig.tabs) do frame:Hide() end
    for c, frame in pairs(Captions) do frame.text:SetTextColor(1, 1, 1, .7) end
...
local function RefreshCaption(i, caption)
    if Tabs[caption] == nil then
        ...
        Captions[caption]:SetScript("OnClick", function() CaptionClick(caption) end)
        Captions[caption]:Show()
    end
end
...
CreateTabsWindow = function(fname)
    ...
    pfExtendConfig.tabs[fname].backdrop = CreateFrame("Frame", "pfExtendConfig" .. fname .. "Backdrop", pfExtendConfig.tabs[fname])
    ...
    pfExtendConfig.tabs[fname].list = pfUI.api.CreateScrollChild("pfExtendConfig" .. fname .. "Scroll", pfExtendConfig.tabs[fname])
...
RefreshTabs = function(caption, i, tab)
    ...
    if Tabs[caption][tab] == nil then
        ...
        local saveType = Tabs[caption][tab].saveType
        if saveType == "number" or saveType == "string" then
            ...
            Tabs[caption][tab].input:SetScript("OnEscapePressed", function(self) this:ClearFocus() end)
        ...
        elseif saveType == "boolean" then
            ...
            Tabs[caption][tab].input:SetScript("OnClick", function()
                if change[caption] == nil then change[caption] = {} end
                change[caption][tab] = this:GetChecked() and true or false
            end)
        elseif saveType == "table" then
            Tabs[caption][tab].input = pfUI.api.CreateDropDownButton("Config_DropDown_" .. caption .. "_" .. tab, Tabs[caption][tab])
            ...
            Tabs[caption][tab].input:SetMenu(function()
                local menu = {}
                for i2, k in ipairs(PfExtend_Config_Template[caption][tab]["selectTable"]) do
                    local entry = {}
                    entry.text = pfExtend_Loc["Config_" .. caption .. "_" .. tab .. "_" .. k]
                    entry.func = function()
                        pfExtend_change_callback(caption, tab, PfExtend_Config_Template[caption][tab]["selectTable"][this.id])
                    end
                    if PfExtend_Global.ReadSetting(caption, tab) == k then
                        Tabs[caption][tab].input.current = i2
                    end
        ...
        elseif saveType == "function" and type(PfExtend_Config_Template[caption][tab]()) == "table" then
...
pfExtendConfig:SetScript("OnMouseDown", function() this:StartMoving() end)
pfExtendConfig:SetScript("OnMouseUp",   function() this:StopMovingOrSizing() end)
pfExtendConfig:SetScript("OnUpdate", function() end)
...
-- ============================================================
-- Registro del frame de eventos principal (ShowLoots + QuestHelper)
-- ============================================================
local pfExtendEvents = CreateFrame("Frame", "pfExtendEventsFrame", UIParent)
pfExtendEvents:RegisterEvent("PLAYER_ENTERING_WORLD")
pfExtendEvents:RegisterEvent("UPDATE_MOUSEOVER_UNIT")
pfExtendEvents:RegisterEvent("WORLD_MAP_UPDATE")

pfExtendEvents:SetScript("OnEvent", function()
