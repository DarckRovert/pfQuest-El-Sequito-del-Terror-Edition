-- ============================================================
--  pfQuest :: Extend Module :: ShowLoots Browser
--  Fuente: Cliencer/pfExtend v1.0.5
--  Adaptado para integración en pfQuest - El Séquito del Terror
-- ============================================================

-- NOTE: pfExtend_Path apunta a Interface\AddOns\pfQuest
-- Los assets TGA están en pfQuest\extend\compat\

local compat = pfExtendCompat
local tooltip_limit = 5
local currentSortBy = "chance"
...
local windowWidth = PfExtend_Global.ReadSetting("ShowLoots","showIds") and (windowWidthWithoutID+60) or windowWidthWithoutID
...
local function ResultButtonEnterSpecial()
        ...
        if items[id]["O"] then
            ...
            for objectID, chance in pairs(items[id]["O"]) do
                ...
                if objects[objectID] and not skip then
                    ...
                    local zone = objects[objectID].coords and objects[objectID].coords[1] and objects[objectID].coords[1][3]
                    GameTooltip:AddDoubleLine(name, (zone and pfMap:GetMapNameByID(zone) or UNKNOWN), 1,1,1,.5,.5,.5)
                end
            end
        end
    ...
    if count > tooltip_limit then
        GameTooltip:AddLine("\n" .. pfExtend_Loc["ToolTips_and"] .. " " ..
            (count - tooltip_limit) .. " " .. pfExtend_Loc["ToolTips_others"], .8, .8, .8)
    end
    GameTooltip:Show()
end
...
local function ResultButtonReload(self)
    ...
    local chanceColor = "|c" .. string.format("%02x%02x%02x%02x", 255,
        self.chanceR * 255, self.chanceG * 255, self.chanceB * 255)
...
local function ResultButtonCreate(i)
    ...
    f.fav.icon:SetTexture(pfExtend_Path .. "\\extend\\compat\\fav")
    ...
    for button, settings in pairs(buttons) do
        ...
        f[button].icon:SetTexture(pfExtend_Path .. "\\extend\\compat\\" .. settings.icon)
...
PFEXShowLoots.Browser:SetScript("OnMouseDown", function() this:StartMoving() end)
PFEXShowLoots.Browser:SetScript("OnMouseUp",   function() this:StopMovingOrSizing() end)
...
PFEXShowLoots.Browser.title:SetText("|cff33ffccpf|rQuest-" .. pfExtend_Loc["windowTitle_ShowLoots"])
...
PFEXShowLoots.Browser.close.texture = PFEXShowLoots.Browser.close:CreateTexture("pfQuestExtendCloseTex")
PFEXShowLoots.Browser.close.texture:SetTexture(pfExtend_Path .. "\\extend\\compat\\close")
...
PFEXShowLoots.Browser.close:SetScript("OnClick", function() this:GetParent():Hide() end)
EnableTooltips(PFEXShowLoots.Browser.close, { pfExtend_Loc["Close"], pfExtend_Loc["Hide browser window"] })
...
PFEXShowLoots.Browser.search.texture:SetTexture(pfExtend_Path .. "\\extend\\compat\\tracker_search")
...
EnableTooltips(PFEXShowLoots.Browser.search, { pfExtend_Loc["Search"], pfExtend_Loc["Show pfQuest Browser"] })
...
PFEXShowLoots.Browser.setting.texture:SetTexture(pfExtend_Path .. "\\extend\\compat\\tracker_settings")
...
EnableTooltips(PFEXShowLoots.Browser.setting, { pfExtend_Loc["Setting"], pfExtend_Loc["Open Config Window"] })
...
local function CreateSortButton(name, textKey, sortBy, xOffset)
    ...
    btn:SetScript("OnClick", function()
        if currentSortBy == sortBy then
            ...
            currentSortOrder = (sortBy == "id") and "asc" or "desc"
        end
        for _, bname in ipairs({ "sortByChance", "sortById", "sortByQuality" }) do
            local b = PFEXShowLoots.Browser[bname]
            if b then
...
local function SortLootList(lootList)
    ...
    if currentSortBy == "chance" then
        sortFunc = currentSortOrder == "desc"
            and function(a,b) return a[2] > b[2] end
            or  function(a,b) return a[2] < b[2] end
    elseif currentSortBy == "id" then
        sortFunc = currentSortOrder == "asc"
            and function(a,b) return a[1] < b[1] end
            or  function(a,b) return a[1] > b[1] end
    elseif currentSortBy == "quality" then
        sortFunc = function(a, b)
            local qa = PfExtend_Database["ShowLoots"]["itemQualityData"][a[1]] or 0
            local qb = PfExtend_Database["ShowLoots"]["itemQualityData"][b[1]] or 0
            return currentSortOrder == "desc" and (qa > qb) or (qa < qb)
        end
    end
    table.sort(starred, sortFunc)
    table.sort(normal,  sortFunc)
    local result = {}
    for _, l in ipairs(starred) do table.insert(result, l) end
    for _, l in ipairs(normal)  do table.insert(result, l) end
    return result
end
...
PFEXShowLoots.Browser:SetScript("OnShow", function()
    ...
    local sortedList = SortLootList(PFEXShowLoots.LootListShown)
    local i = 0
    for _, l in ipairs(sortedList) do
