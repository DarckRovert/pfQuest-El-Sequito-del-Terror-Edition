-- ============================================================
--  pfQuest :: Extend Module :: ShowLoots Main Logic
...
-- Construye la base de datos de botín indexada por mob/objeto
            ...
            for from, probability in pairs(LootData) do
                if probability > 0 then
                    if lootType == "U" or lootType == "O" then
                        if db[lootType] == nil then db[lootType] = {} end
                        if db[lootType][from] == nil then db[lootType][from] = {} end
                        ...
                        if pfDB["refloot"]["data"][from] then
                            for refLootType, refLootData in pairs(pfDB["refloot"]["data"][from]) do
                                for refFrom, refProbability in pairs(refLootData) do
                                    if db[refLootType] == nil then db[refLootType] = {} end
                                    if db[refLootType][refFrom] == nil then db[refLootType][refFrom] = {} end
...
-- Lee la tabla de drops para el mob bajo el cursor
PFEXShowLoots.ModifyTooltip = function()
    ...
    PFEXShowLoots.focus_name = getglobal("GameTooltipTextLeft1") and
        getglobal("GameTooltipTextLeft1"):GetText() or "__NONE__"
    ...
    for id in pairs(pfDatabase:GetIDByName(PFEXShowLoots.focus_name, "units")) do
        if pfDB["units"]["data"][id] and pfDB["units"]["data"][id]["coords"] then
            for _, data in pairs(pfDB["units"]["data"][id]["coords"]) do
                ...
                if zone == focus_zone and db["U"] and type(db["U"][id]) == "table" then
                    ...
                    for _, v in ipairs(PfExtend_Global.sortKeyValueTable(sortLootList, "value", true)) do
                        if v.value > 100 then v.value = v.value - 100 end
...
-- Detecta Alt+Ctrl para abrir el browser de drops
...
-- Inyecta los drops en el tooltip del juego
pfMap.tooltip:SetScript("OnUpdate", function()
    ...
    if not isShown then
        ...
        elseif miniq > 0 and i == 0 then
            GameTooltip:AddLine(string.format(pfExtend_Loc["No %s or better loots"],
                pfExtend_Loc["Config_ShowLoots_itemQualityFilter_" .. miniq] .. "|r"), 0.55, 0.55, 0.55)
        elseif miniq > 0 and i > 0 then
            GameTooltip:AddLine(string.format(pfExtend_Loc["%d %s or better loots(of %d)"], j,
        ...
        for _, line in pairs(showlines) do
            GameTooltip:AddLine(
                line.itemLink .. " |cff555555[|r" .. string.format("%.2f", line.chance) .. "%|cff555555]",
...
function PFEXShowLoots.OnLoad()
    -- vacío — carga dinámica
end

function PFEXShowLoots.OnEvent(event, arg1)
    if not PfExtend_Global.ReadSetting("ShowLoots", "enable") then return end
    if event == "PLAYER_ENTERING_WORLD" then
        if not PfExtend_Database["ShowLoots"]["updated"] or
