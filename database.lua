...
-- Return the best cluster point for a coordinate table
...
for id, db in pairs(dbs) do
  ...
  -- Free turtle patch data (already merged by patchtable.lua — but patchtable runs AFTER
...
pfDatabase.tracking:SetScript("OnEvent", function()
  ...
  -- Clean up legacy SavedVariable from earlier versions that accidentally
  -- stored collapsedZones in pfQuest_track
  if pfQuest_track.collapsedZones then
    pfQuest_track.collapsedZones = nil
  end

  -- build static reject set now that all addon Lua is loaded
...
-- for exact matches (the hot path in SearchQuestID).
...
function pfDatabase:BuildStaticRejectSet()
  ...
  for id in pairs(quests) do
    ...
    elseif prace and quests[id]["race"] and not (band(quests[id]["race"], prace) == prace) then
      reject[id] = true
    -- wrong class (only when pclass is known)
    elseif pclass and quests[id]["class"] and not (band(quests[id]["class"], pclass) == pclass) then
      reject[id] = true
    end
  end
end
...
function pfDatabase:QuestFilter(id, plevel, pclass, prace)
  ...
  -- hide lowlevel quests (ignore filter at level 60)
